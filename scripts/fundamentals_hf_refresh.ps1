...
# (--no-cache defeats the pipeline's own HF-cache short-circuit, forcing a real
# re-download + re-push), rebuilds curated, then verifies the HF push actually
# landed via verify_hf.py before declaring success. Registered as Windows
# Scheduled Task "ClaudeAuto-FundamentalsHFRefresh" (see AUTOMATION.md).
...
cmd /c "`"$py`" fundamentals_pipeline.py --full-market --no-cache > `"$report`" 2>&1"
...
$exit = [Math]::Max([Math]::Max($pipelineExit, $curatedExit), $verifyExit)

$summary = (Select-String -Path $report -Pattern "^(VERIFY PASS|VERIFY FAIL)$" | Select-Object -Last 1).Line
if (-not $summary) { $summary = "no VERIFY line - a step crashed before verify_hf.py ran? (pipeline=$pipelineExit curated=$curatedExit verify=$verifyExit)" }
...
if ($exit -eq 0) {
    ...
    $status = "FAIL"
    "Fundamentals HF refresh FAILED on $stamp (pipeline=$pipelineExit curated=$curatedExit verify=$verifyExit).`n$summary`nFull report: $report" |
