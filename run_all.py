...
PIPELINES: list[PipelineSpec] = [
    ...
    PipelineSpec(
        ...
        tables=["eia_petroleum_stocks", "eia_natgas_storage", "eia_crude_production",
                "eia_refinery_activity", "eia_crude_trade", "eia_petroleum_demand"],
        requires_env=["EIA_API_KEY"],
        backfill_args=["--backfill"],
        timeout=600,
    ),
    ...
    PipelineSpec(
        ...
        desc="California legislature Form 700 schedules A-1/A-2/B/C/D/E (investments, business entities/trusts, real property, income/loans, gifts, travel payments) -- FPPC eRetrieval (keyless)",
        stage=1,
        tables=["california_disclosures", "california_disclosures_business",
        ...
        backfill_args=["--backfill", "--schedules", "a1", "a2", "b", "c", "d", "e"],
        incremental_args=["--schedules", "a1", "a2", "b", "c", "d", "e"],
        timeout=300,
    ),
    PipelineSpec(
        name="patents",
    ...
    PipelineSpec(
        name="cboe_strategy",
        file="cboe_strategy_pipeline.py",
        desc="CBOE option-strategy benchmark indices: PUT, BXM, CNDR, CLL (keyless)",
        stage=1,
        tables=["cboe_strategy_indices"],
        backfill_args=["--backfill"],
        timeout=180,
    ),
    PipelineSpec(
        name="policy_rates",
        file="policy_rates_pipeline.py",
        desc="BIS central bank policy rates, 39 areas, daily + monthly (keyless)",
        stage=1,
        tables=["policy_rates"],
        backfill_args=["--frequency", "both"],
        timeout=420,
    ),
    PipelineSpec(
    ...
    PipelineSpec(
        ...
        desc="NY Fed GSCPI + FRED multi-modal (marine/rail/truck/air) freight PPI + diesel fuel PPI + EIA diesel waterborne trade",
        stage=1,
        tables=["shipping_gscpi", "shipping_freight_ppi", "shipping_diesel_trade"],
        requires_env=["FRED_API_KEY", "EIA_API_KEY"],
        backfill_args=["--backfill"],
        timeout=300,
    ),
    PipelineSpec(
    ...
    PipelineSpec(
        ...
        file="proshares_holdings_pipeline.py",
        desc="ETF holdings -- ProShares daily holdings master CSV (all ProShares ETFs, no auth) (Iceberg table)",
        stage=1,
        tables=['fund_holdings'],
        timeout=300,
    ),
    PipelineSpec(
        name="vaneck_holdings",
        file="vaneck_holdings_pipeline.py",
        desc="ETF holdings -- VanEck daily holdings XLSX (official issuer, no auth) (Iceberg table)",
        stage=1,
        tables=['fund_holdings'],
        timeout=300,
    ),
    PipelineSpec(
        name="globalx_holdings",
        file="globalx_holdings_pipeline.py",
        desc="ETF holdings -- Global X daily holdings CSV (official issuer CDN, no auth, rate limited) (Iceberg table)",
        stage=1,
        tables=['fund_holdings'],
        timeout=600,
    ),
    ...
    PipelineSpec(
        name="amplify_holdings",
        file="amplify_holdings_pipeline.py",
        desc="ETF holdings -- Amplify daily holdings (public Firestore feed, keyless) (Iceberg table)",
        stage=1,
        tables=['fund_holdings'],
        timeout=300,
    ),
    PipelineSpec(
        name="firsttrust_holdings",
        file="firsttrust_holdings_pipeline.py",
        desc="ETF holdings -- First Trust daily holdings (server-rendered ASP.NET page, keyless) (Iceberg table)",
        stage=1,
        tables=['fund_holdings'],
        timeout=300,
    ),
    PipelineSpec(
        name="direxion_holdings",
        file="direxion_holdings_pipeline.py",
        desc="ETF holdings -- Direxion daily holdings CSV (official issuer, keyless, no rate limiting) (Iceberg table)",
        stage=1,
        tables=['fund_holdings'],
        timeout=300,
    ),
