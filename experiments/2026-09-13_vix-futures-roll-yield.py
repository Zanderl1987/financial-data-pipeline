...
def main(days: int = 7, output_dir: str = None) -> None:
    """Run the VIX futures roll-yield signal backtest on the last N days."""
    ...
    # Determine date window: last N calendar days from today (2026-09-13)
    ...
    if not vix_df.empty and not results_df.empty:
        ...
        # Also test 5-day and 10-day forward returns via rolling
        # (simplified: just report the 1d IC; multi-day would require cumulative return calculation)
...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    main(days=args.days, output_dir=args.output)