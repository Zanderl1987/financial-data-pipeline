...
| **Schwab** | OAuth re-auth every 7 days: browser login required (cannot be fully unattended), but `scripts\schwab_reauth.py` captures the redirect locally (127.0.0.1:8182, cert trusted) so no paste is needed — see SESSION_NOTES_2026-08-11.md. Trader API (positions/transactions) needs separate enablement at developer.schwab.com beyond the base Market Data API. | ongoing (reauth done 2026-08-11, next 2026-08-18) |
...
| **Schwab `price_history` returns additively dividend/split-adjusted closes — can go negative for old dates** | Not a pipeline bug; confirmed via raw pre-curation data. For long-lived dividend-paying names (COST, PGR, PSA, CME, FAST, TROW, TDG, PCAR, and 100+ others), Schwab's historical `close` appears to be back-adjusted by *subtracting* cumulative dividends rather than a ratio-based split/dividend adjustment — once cumulative dividends exceed the nominal historical price, the adjusted close goes negative. Spot-checked COST: -$28 in 1986, climbing steadily to -$0.56 by 2009-03-09, crossing positive the very next trading day, reaching a normal $779 by 2024. 161,783 rows (0.34% of the `prices` table) have `close < 0`, spanning 106+ symbols, dates 1985-2025. **Current/latest data is clean** — checked every symbol's most recent bar; only `SVA` (Sinovac Biotech, halted/delisted ~2019) is still negative at its last available date, everything else resolves positive well before the data's edge. `pct_change`/`log_return` stay meaningful straight through the crossing point (that continuity is the actual point of back-adjustment) — but anything treating `close` as a literal historical dollar price (e.g. `analytics/signals.py`'s `value` factor doing `fund_eps/close`, or an absolute-price event study reaching deep into a symbol's history) will get nonsense for dates before that symbol's crossing point. Not fixed/guarded yet — flagged for whoever touches `value` or deep-history event studies next. Related, smaller-scale artifacts from the same full-history pull: 4 symbols (`SPGI`, `HWM`, `NC`, `CAG`) have runs of implausible-tiny-magnitude bars from 1970-1979 predating when those tickers/entities could plausibly have existed under that name (same adjustment mechanism hitting an extreme, most likely); 142 symbols (83% OTC) show a 5+ year gap between their 1st and 2nd recorded bar, i.e. a stale reference quote followed by real (sparse) trading — don't trust "years of history" for those without checking. | 2026-07-25, full DuckDB-scale audit of the 46.9M-row `prices` table |
